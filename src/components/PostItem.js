  ...
  function sendComment(e) {
    dispatch(
      addComment({
        postId: postId,
        newComment: {
          userId: localStorage.getItem("UserId"), userFullname: localStorage.getItem("UserFirstName") + " " +  localStorage.getItem("UserLastName"),  content: commentContent,
        },
      })
    );
    setCommentContent("");
  ...
  return (
    <div className="border shadow rounded-3 border-primary p-3 mt-3" style={{color: "#D7DF22"}}>
      <Row>
        ...
        {/*comments and comment input box list*/}
        {commentStatus === true ? (
          <div className="mt-3">
            <div className="d-flex align-items-center">
              <Form className="w-100 mx-1">
                <Form.Group>
                  <Form.Control type="text"  placeholder="Write a comment..." value={commentContent}  onChange={handleCommentContentChange} />
                </Form.Group>
              </Form>
